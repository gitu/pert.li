...
The chat handler auto-detects the LLM provider (Anthropic → OpenAI → Gemini, by key presence). To use a different one, set `LLM_PROVIDER=openai|anthropic|gemini` and (optionally) `LLM_MODEL=<id>`. The OpenAI adapter accepts an `OPENAI_BASE_URL` override so it can talk to any OpenAI-compatible `/v1` endpoint — Azure OpenAI, OpenRouter, LM Studio, Ollama, vLLM, llama.cpp, corporate LLM gateways. Defaults to `https://api.openai.com/v1` when unset. When a custom base URL is set, requests use the classic `/v1/chat/completions` wire format instead of OpenAI's newer `/v1/responses` (override with `OPENAI_API_FORMAT=responses|chat-completions`).

Tool schemas are rewritten per provider before reaching the adapter (`src/lib/ai/gemini-compat.ts`, `src/lib/ai/openai-compat.ts`, shared walker in `src/lib/ai/schema-compat.ts`): Gemini rejects JSON Schema `const`, and OpenAI strict function calling rejects `oneOf` (which Zod emits for `z.discriminatedUnion`, e.g. `propose_changes`' operations). New chat tools that use unions/literals get this for free; new providers need their own compat module if their schema dialect is restricted.
...
6. **Storybook coverage rules.** Every new or touched reusable component in `src/components/` ships with a `*.stories.tsx` covering the default plus meaningful variants (loading/empty/error/interactive). Use `play` functions for interaction assertions when the component has non-trivial behavior. Storybook runs through its own isolated vite config (`.storybook/vite.config.ts`) — don't let it pick up the root `vite.config.ts` (Nitro's `configureServer` will crash). Don't backfill stories for unmodified files.
   - **Visual-regression baseline.** A `main` baseline of every story screenshot lives on the `screenshots` orphan branch under `baseline/` (refreshed by the `baseline-screenshots` job in `storybook.yml`, committing only changed images). PRs render all stories and pixel-diff them against that baseline via `scripts/diff-screenshots.mjs` (tolerance knobs: `SCREENSHOT_DIFF_THRESHOLD` / `_RATIO` / `_MIN_PIXELS`); the `pr-screenshots` job posts a sticky comment showing only the stories that changed (after + diff overlay). If a story is legitimately non-deterministic across builds (auto-layout like the `PERT/Canvas` xyflow/elk graph, freshly-generated ids, timestamps), tag it `tags: ["no-screenshot-diff"]` on the story or its meta — the diff skips it (and logs that it did) while the `storybook` test-runner still runs its play function.
