...
const SYSTEM_PROMPT = [
	...
	"      Branch — relocated here from the header), and 'jump-off' cards into",
	"      each of the other four views.",
	...
	"  menu that forks the current plan into a sibling 'branch' project (a new project row,",
	"  same workspace, cloned Automerge doc with parent metadata). You can",
	"  branch a branch, so plans nest to any depth. The same menu also exposes",
	"  'Rename / edit description' to change either field in place, and — when",
	"  the current plan is itself a branch — 'Promote to standalone plan',",
	"  which detaches it from its parent so it's no longer a branch (its own",
	"  sub-branches stay attached to it). The same Rename and Promote actions",
	"  are on each project card on the workspace home (`/`) via a '⋯' menu,",
	"  where the description also shows as a muted second line under the title.",
	"  Branches show up nested under their parent — recursively, branch-of-a-",
	"  branch included — both in the sidebar and on the workspace home (where",
	"  they appear as indented cards), each with a small Git-branch glyph and",
	"  their optional description as a muted second line. Inside a branch a thin banner above the canvas",
